...
u8 motor_disturb =0,Continue_disturb = 0,in_reset = 0;
...
//		if(!UART1_Overflow_Flag && (!in_reset))													//没数据就进来
//		{
				
			if( UART1_Flag_count < UART_BUF_LEN)
			{
				UART1_Recv_Buf[UART1_Flag_count] = USART1 ->DR ; 					////赋值,清零;将数据寄存器的值存放到 UART1_Recv_Buf
				//USART1 ->DR = UART1_Recv_Buf[UART1_Flag_count];  					//返还回来.回显				{调试时使用后期可屏蔽}
				send_Byte(UART1_Recv_Buf[UART1_Flag_count]);

				UART1_Flag_count++;    
				if(UART1_Recv_Buf[UART1_Flag_count-1] == '\n' || UART1_Recv_Buf[UART1_Flag_count-2] == '\r' || UART1_Flag_count >= 7 )	//PELCO_D格式数据7位~
				{		
					UART1_Overflow_Flag = 1;													//关闭串口中断,停止接收 并跳到主函数执行语句
//					UART1_Recv_Buf[UART1_Flag_count] = '\0';
//					Send_string(UART1_Recv_Buf); //显示字符串
